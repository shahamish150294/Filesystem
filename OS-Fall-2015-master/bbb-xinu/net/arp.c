...
void arp_removestale()
{	int sl,curtime;
	struct arpentry *ar;	
	for (sl=0; sl < ARP_SIZ; sl++) {
		ar=&arpcache[sl];
		if (ar->arstate == AR_RESOLVED) {
			curtime=clktime;
			if((curtime-ar->staletime)>300)	
				{ar->arstate=AR_FREE;
				printf("\n Entry %d deleted",ar->arpid);
				}
		}
	}
}
	...
	)
{
	...
	if (found) {
		...
		if (arptr->arstate == AR_PENDING) {
			...
			send(arptr->arpid, OK);
			/////////////////////////
			arptr->staletime = clktime;
			printf("Entry resolved with time %d",arptr->staletime);
		}
	}
